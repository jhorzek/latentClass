...
latentClass <- function(data,
  ...
  model <- LCMR$new(class_probabilities,
                    nrow(data))
  ...
  # Optimize
  timing <- system.time({converged <- model$expectation_maximization(opt_settings$max_iter,
...
initialize_normal <- function(data,
  ...
  if(is.list(normals$starting_values)){
    ...
    for(item in normals$items){
      ...
      starting_values[[item]][1, ] <- stats::runif(n = n_classes,
                                                   min = stats::quantile(data[[item]], .05, na.rm = TRUE),
                                                   max = stats::quantile(data[[item]], .95, na.rm = TRUE))
      if(normals$sd_equal[item]){
        starting_values[[item]][2, ] <- rep(.5*stats::sd(data[[item]], na.rm = TRUE), n_classes)
      }else{
        starting_values[[item]][2, ] <- stats::runif(n = n_classes,
...
initialize_categorical <- function(data,
  ...
  if(is.list(categoricals$starting_values)){
    ...
    for(item in categoricals$items){
      ...
      starting_values[[item]] <- matrix(stats::runif(n = length(item_levels)*n_classes, min = .1, 1), 
